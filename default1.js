...
function upload1(){
 ...
 try{ this.getDonationStatus();}catch(e){}
...
//   e.preventDefault();
    ...
    firebase.database().ref(`images/Sponsers/GOLD`).once('value').then(function(snapshot){
      ...
      for(let[key,value] of Object.entries(data)){
        ...
        "<img style='background-color:white;width:100%' src='"+value.imageURL+"' >"+
    ...
    firebase.database().ref(`images/Sponsers/GOLD`).once('value').then(function(snapshot){
      ...
      for(let[key,value] of Object.entries(data)){
        ...
        "<img style='background-color:white' src='"+value.imageURL+"' >"+
        // "<div ><h3 >"+value.heading+"</h3>"+
        // "<div ><p >"+value.text+"</p>"+
        //  "<button  onclick=window.open('"+value.Website+"') style='margin-bottom:30px;width:95%!important'>Learn more</button>"+
        "</a><br/>"+
        "<button class='btn btn-danger' id='"+key+"' onclick='deleteGold_post(this.id)' style='margin-bottom:30px;width:100px!important' >Delete</button>"+"</div>"+
        posts1_div.innerHTML;
      }
    }
    catch(e){}
    ...
    firebase.database().ref(`images/Sponsers/SILVER`).once('value').then(function(snapshot){
      ...
      for(let[key,value] of Object.entries(data)){
        ...
        "<a target='_blank' class='image featured' href='"+value.Website+"' style='border:none ;' >"+
        "<img style='background-color:white' src='"+value.imageURL+"' style='justify-content:center' >"+
        // "<div ><h3 >"+value.heading+"</h3>"+
        // "<div ><p >"+value.text+"</p>"+
        //  "<button  onclick=window.open('"+value.Website+"') style='margin-bottom:30px;width:95%!important'>Learn more</button>"+
        //  "<button class='hidden2' id='"+key+"' onclick='delete_post(this.id)' style='margin-bottom:30px;'>Delete</button>"+
    ...
    firebase.database().ref(`images/Sponsers/SILVER`).once('value').then(function(snapshot){
      ...
      for(let[key,value] of Object.entries(data)){
        ...
        "</a><br/>"+ "<button class='btn btn-danger' id='"+key+"' onclick='deleteSilver_post(this.id)' style='margin-bottom:30px;width:100px!important'>Delete</button>"+"</div>"+
        Silverposts1_div.innerHTML;
      }
    }
    catch(e){}
    ...
    firebase.database().ref(`images/Sponsers/BRONZE`).once('value').then(function(snapshot){
      ...
      for(let[key,value] of Object.entries(data)){
        ...
        "</a><br/>"+"<button class='btn btn-danger' id='"+key+"' onclick='deleteBronze_post(this.id)' style='margin-bottom:30px;width:100px!important'>Delete</button>"+"</div>"+
        Bronzeposts1_div.innerHTML;
      }
    }
    catch(e){}
...
try{
  ...
  {
{
    firebase.database().ref('ShowingDonation/status').once('value').then(function(snapshot){
        ...
        if (data=='true') {
            // alert("sahi hai");
          try { mainDonation.style.display="block"}catch(e){}
          //  donationButton.innerHTML="Hide Donation"
   
           
        } else
        {
          
         try { mainDonation.style.display="none"}catch(e){}
        }
              ...
              for(let[key,value] of Object.entries(data)){
               ...
               " <li>"+
													"<article class='box post-excerpt'>"+
														...
														"<h3><a target='_blank' href="+value.Website+">Robocon "+value.heading+" theme</a></h3>"+
														"<p>"+value.text+" <br></p>"+
												"	</article>"+
										"</li>"
                +posts_div.innerHTML;
